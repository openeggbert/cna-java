...
/** Dynamic vertex buffer whose streaming options and ContentLost contract are backed by CNA. */
...
public class DynamicVertexBuffer extends VertexBuffer {
    ...
    public DynamicVertexBuffer(
            ...
            int vertexCount,
            BufferUsage usage) {
        super(graphicsDevice, vertexDeclaration, vertexCount, usage, true);
    }
    ...
    public void addContentLostListener(EventHandler<EventArgs> listener) {
        ...
        if (contentLostRegistration == 0L) {
            contentLostRegistration = NativeBindings.subscribeVertexBufferContentLost(this, this);
        }
    ...
    public final boolean getIsContentLost() {
        ensureNotDisposed();
        rethrowContentLostFailure();
        return NativeBindings.getVertexBufferIsContentLost(this);
    }
    ...
    public final <T> void SetData(
        ...
        setData(-1, data, startIndex, elementCount,
                getVertexDeclaration().getVertexStride(), options);
    }
    ...
    public final <T> void SetData(
            ...
            SetDataOptions options) {
        rethrowContentLostFailure();
        setData(offsetInBytes, data, startIndex, elementCount, vertexStride, options);
    }
    ...
    void releaseDynamicSubscription() {
        ...
        if (registration != 0L) {
            NativeBindings.unsubscribeVertexBufferContentLost(registration);
